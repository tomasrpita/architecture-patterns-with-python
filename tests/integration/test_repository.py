...
def test_repository_can_retrieve_a_batch_with_allocations(session):
    ...
    # * So we also explicitly check on its major attributes,
    # including ._allocations, which is a Python set of OrderLine value objects.
...
def add_order_line(session, orderid, sku, qty):
    session.execute(
        "INSERT INTO order_lines (orderid, sku, qty)"
...
def test_updating_a_batch(session):
    add_order_line(session, "order1", "WEATHERED-BENCH", 10)
    add_order_line(session, "order2", "WEATHERED-BENCH", 20)
